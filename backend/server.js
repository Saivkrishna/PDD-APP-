...
function processLiveJobMarket(apiJobs) {
  ...
  apiJobs.forEach(job => {
    ...
    if (Array.isArray(job.tags)) {
      tags = job.tags.map(t => (t && typeof t.toLowerCase === 'function') ? t.toLowerCase() : '');
    ...
    const matches = (keywords) => {
      return keywords.some(k => title.includes(k) || desc.includes(k) || tags.some(t => typeof t === 'string' && t.includes(k)));
    };
...
app.get('/api/search', (req, res) => {
  ...
  try {
    ...
    // 9. Trending Careers
    const activeTrending = (typeof liveTrendingJobs !== 'undefined' && Array.isArray(liveTrendingJobs) && liveTrendingJobs.length > 0) 
      ? liveTrendingJobs 
      : defaultTrending;

    activeTrending.forEach(career => {
      if (
        career.title.toLowerCase().includes(q) || 
        (career.description && career.description.toLowerCase().includes(q)) ||
        (Array.isArray(career.skills) && career.skills.some(s => s.toLowerCase().includes(q)))
      ) {
        results.push({
          ...
          icon: career.icon || '🔥',
          id: career.id,
          payload: { type: 'trending', jobId: career.id }
        });
      }
    });
  } catch (err) {

      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {channels && channels.te && channels.te.name && channels.te.url && (
              <div
...
export default function App() {
  const [showSplash, setShowSplash] = useState(() => !localStorage.getItem('cp_user'));
      ...
      if (firebaseUser) {
        ...
        try {
          if (cached) {
            ...
            if (parsed && parsed.id && parsed.id.startsWith('dev-uid-')) {
              isBypass = true;
            }
          }
        } catch (e) {}
        ...
        return (
          <SearchPage
            ...
            onSelectResult={(result) => {
              playClickSound(soundEnabled);
              handleNavigateToPayload(result);
            }}
            onOpenSettings={() => handleNavChange('settings')}
          />
        );
  ...
  if (!user) {
    if (authScreen === 'login') {
      return (
        <LoginPage
          onLogin={(u) => { playClickSound(soundEnabled); setUser(u); if (u) localStorage.setItem('cp_user', JSON.stringify(u)); }}

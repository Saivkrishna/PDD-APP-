...
import './AIWorkspace-saikrishna.css';

const API = process.env.REACT_APP_API_URL || '/api';

const enforceChatLimit = (chatsList) => {
  if (chatsList.length <= 10) return chatsList;
  const pinned = chatsList.filter(c => c.pinned).sort((a, b) => b.updatedAt - a.updatedAt);
  const unpinned = chatsList.filter(c => !c.pinned).sort((a, b) => b.updatedAt - a.updatedAt);
  const result = [];
  result.push(...pinned);
  if (result.length < 10) {
    result.push(...unpinned.slice(0, 10 - result.length));
  } else {
    result.splice(10);
  }
...
export default function AIWorkspace({
  ...
  const [chats, setChats] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) return enforceChatLimit(parsed);
      }
    } catch (e) {}
    return [
      {
  ...
  const handleCreateNewChat = () => {
    ...
    setChats(prev => enforceChatLimit([newChat, ...prev]));
      ...
      <div className="ai-main-area">
        ...
        <div className="ai-header">
          <div className="ai-title-wrap">
            {onClose && isPageMode && (
              <div className="ai-tooltip-container">
                ...
                <span className="ai-tooltip-text" style={{ left: 0, right: 'auto' }}>Go Back</span>
              </div>
            )}
            <div className="ai-tooltip-container">
              <button className="ai-history-toggle-btn" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
                📜 History
              </button>
              <span className="ai-tooltip-text" style={{ left: 0, right: 'auto' }}>Toggle History</span>
            </div>
          ...
          <div className="ai-header-controls">
            <div className="ai-tooltip-container">
              <button className="ai-new-chat-header-btn" onClick={handleCreateNewChat}>
                ➕ New Chat
              </button>
              <span className="ai-tooltip-text" style={{ left: '50%', right: 'auto', transform: 'translateX(-50%)' }}>New Chat</span>
            </div>
            <div className="ai-tooltip-container">
              <button className="ai-icon-btn" onClick={() => handleExportChat()}>
                📤
              </button>
              <span className="ai-tooltip-text" style={{ left: '50%', right: 'auto', transform: 'translateX(-50%)' }}>Export Chat</span>
            </div>
            <div className="ai-tooltip-container">
              <button className="ai-icon-btn" onClick={() => setIsFullscreen(!isFullscreen)}>
                {isFullscreen ? '🗗' : '⛶'}
              </button>
              <span className="ai-tooltip-text" style={{ left: '50%', right: 'auto', transform: 'translateX(-50%)' }}>
            ...
            {onClose && !isPageMode && (
              <div className="ai-tooltip-container">
                ...
                <span className="ai-tooltip-text" style={{ left: 'auto', right: 0 }}>Close Workspace</span>
              </div>
            )}
          </div>
        </div>
